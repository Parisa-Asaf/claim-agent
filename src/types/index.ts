...
// ─── Feature 3 (M3): Recovery Dashboard ──────────────────────────────────────
export interface DashboardStats {
  totalClaims: number;
  activeClaims: number;
  settledClaims: number;
  totalPotentialRefund: number;       // BDT
  totalRecovered: number;             // BDT
  recoveryRate: number;               // 0–100 percent
  highPriorityClaims: number;
  expiringSoon: number;               // within 7 days
  successRate: number;                // % of closed claims that were settled
}
...
export interface DashboardApiResponse {
  ...
  countdowns?: ClaimCountdown[];
  error?: string;
}

// ─── Feature 4 (M3): AI Settlement Assistant ─────────────────────────────────
export type SettlementVerdict = "FAIR" | "UNFAIR" | "PARTIAL" | "INVESTIGATE";
